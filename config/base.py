...
def get_config():
    ...
    # Per-step solver used for both sampling and the OPD per-step KL.
    # One of {"flow", "dance", "ddim", "dpm1", "dpm2"}; see
    # ``flow_grpo/diffusers_patch/solver.py`` for details. ``flow`` (the
    # default) is the legacy FlowGRPO SDE Euler step and reproduces
    # pre-multi-solver OPD numbers bit-for-bit.
    sample.solver = "flow"
    # Force SDE-style solvers (flow / dance / ddim) to use η = 0 so the
    # trajectory is fully deterministic. DPM solvers ignore this flag —
    # they are deterministic by construction. Same effect on the OPD
    # loss as `noise_level = 0` (squared-L2 surrogate kicks in), but
    # keeps `noise_level` available as a separate knob for the
    # SDE-style solvers' η scale when desired.
